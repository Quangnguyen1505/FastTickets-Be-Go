...
CREATE TABLE IF NOT EXISTS pre_go_acc_user_verify_9999 (
    verify_id SERIAL PRIMARY KEY,                   -- ID of the OTP record
    ...
    verify_updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP          -- Record update time
);
-- +goose StatementEnd

-- Tạo chỉ mục riêng sau khi bảng đã được tạo
-- +goose StatementBegin
CREATE INDEX idx_verify_otp ON pre_go_acc_user_verify_9999 (verify_otp);
-- +goose StatementEnd
