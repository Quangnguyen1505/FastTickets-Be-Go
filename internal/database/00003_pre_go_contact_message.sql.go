...
// versions:
//   sqlc v1.25.0
// source: 00003_pre_go_contact_message.sql
...
func (q *Queries) CreateContactMessage(ctx context.Context, arg CreateContactMessageParams) (PreGoContactMessage, error) {
...
func (q *Queries) GetAllContactMessages(ctx context.Context, arg GetAllContactMessagesParams) ([]PreGoContactMessage, error) {
	rows, err := q.db.Query(ctx, getAllContactMessages, arg.Column1, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []PreGoContactMessage
	for rows.Next() {
		var i PreGoContactMessage
		if err := rows.Scan(
			&i.ID,
...
SELECT id, name, email, phone, message, status, created_at FROM pre_go_contact_messages
WHERE id = $1
`

func (q *Queries) GetContactMessageByID(ctx context.Context, id pgtype.UUID) (PreGoContactMessage, error) {
